...
export class ForexRateService extends ThirdPartyApiService {
    ...
    constructor(
        ...
        const cachePath =
            options.cachePath ?? path.join(process.cwd(), "cache");
        ...
        console.log("[ForexRateService] Initialized with:", {
            ...
            cacheDuration: `${this.cacheDuration}s (${Math.round(
    ...
    private initializeCache(): void {
        try {
            ...
            if (!fs.existsSync(cacheDir)) {
                ...
                console.log(
                    "[ForexRateService] Created cache directory:",
                    cacheDir
                );
            }
            ...
            if (fs.existsSync(this.cacheFile)) {
                ...
                if (!cacheData || typeof cacheData !== "object") {
                    console.warn(
                        "[ForexRateService] Invalid cache file format, ignoring"
                    );
                    return;
                }
                ...
                console.log(
                    `[ForexRateService] Loaded ${loadedCount} currency rates from cache file`
                );
            } else {
                console.log(
                    "[ForexRateService] No cache file found, will create on first API call"
                );
            }
        } catch (error) {
            logger.error("[ForexRateService] Error initializing cache:", error);
        }
    }
            ...
            if (this.usePersistentCache) {
                ...
                if (fs.existsSync(this.cacheFile)) {
                    try {
                        ...
                        logger.error(
                            "[ForexRateService] Error parsing existing cache file:",
                            error
                        );
                        console.warn(
                            "[ForexRateService] Could not parse existing cache file, creating new one"
                        );
                    }
                }
                ...
                console.log(
                    "[ForexRateService] Updated forex rates in cache file"
                );
            }
        } catch (error) {
            logger.error("[ForexRateService] Error saving to cache:", error);
        }
    }
    ...
    public async getExchangeRates(
        ...
        if (!forceRefresh) {
            ...
            if (
                cachedRates &&
                now - cachedRates.timestamp < this.cacheDuration
            ) {
                console.log(
                    "[ForexRateService] Using cached forex rates for",
                    baseCurrency
                );
                return cachedRates;
            }
            ...
            if (cachedRates) {
                console.log(
                    "[ForexRateService] Cache expired, refreshing data"
                );
            }
        } else {
            console.log(
                "[ForexRateService] Force refresh requested, bypassing cache"
            );
        }
        ...
        while (retries <= this.maxRetries) {
            try {
                ...
                const response = await this.get<ForexRateResponse>(
                    "",
                    queryParams
                );
                ...
                console.log(
                    "[ForexRateService] Successfully fetched forex rates:",
                    {
                        base: response.base,
                ...
                if (retries <= this.maxRetries) {
                    ...
                    console.warn(
                        `[ForexRateService] Retry ${retries}/${this.maxRetries} after ${delay}ms:`,
                        lastError.message
                    );
        ...
        logger.error("[ForexRateService] All API attempts failed:", lastError);

        // After all retries failed, try to use expired cache as fallback
        const cachedRates = this.cache.get<ForexRateResponse>(cacheKey);
        if (cachedRates) {
            console.warn(
                "[ForexRateService] Using expired cached rates as fallback"
            );
            return cachedRates;
        }
        ...
        throw (
            lastError ||
            new Error("Failed to fetch exchange rates after multiple attempts")
        );
    }
